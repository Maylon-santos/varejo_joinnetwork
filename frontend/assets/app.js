...
let lastIndicators=null,permissoes=new Set(),isAdmin=false,userRole='',somenteProprias=false;
const pode=recurso=>permissoes.has(recurso);
const podeConsolidado=()=>['Admin','Diretoria','Supervisao'].includes(userRole)&&!somenteProprias&&pode('consolidado:ler');
const podePagina=next=>next==='consolidated'?podeConsolidado():next==='products'?pode('produtos:ler'):next==='queue'?pode('fila:ler'):['permissions','users','employees'].includes(next)?isAdmin:next==='overview'?(pode('indicadores:ler')||pode('ranking:ler')):next==='quality'?(pode('vendas:ler')&&pode('conferencia:ler')):next==='customers'?(pode('clientes:ler')&&pode('vendas:ler')):next==='sales'&&pode('vendas:ler');
...
function exitSession(message=''){
 goalsGeneration++;goalsAbort?.abort();goalEditing=null;$('goal-dialog').close();for(const id of ['consolidated-body','consolidated-total','consolidated-metrics','goals-body','goals-metrics'])$(id).replaceChildren();userRole='';
 ...
 clearTimeout(reprocessTimer);clearInterval(queueTimer);$('queue-action-dialog').close();$('queue-content').replaceChildren();queueData=null;token=null;generation++;detailGeneration++;abort?.abort();clearInterval(refreshTimer);lastIndicators=null;
...
$('login-form').addEventListener('submit',async e=>{
 ...
 try{
  ...
  userRole=me.usuario.role;somenteProprias=!!me.usuario.somente_proprias_vendas;isAdmin=userRole==='Admin';$('goals-month').value=today().slice(0,7);permissoes=new Set(me.usuario.permissoes||[]);
  ...
  const inicial=['overview','consolidated','sales','quality','queue','products','customers'].find(podePagina);
  ...
  clearInterval(refreshTimer);refreshTimer=setInterval(()=>{if(token&&!['permissions','users','employees'].includes(page)&&!document.hidden&&!$('detail-dialog').open&&!$('product-dialog').open&&!$('goal-dialog').open)loadData();},360000);
...
function setPage(next,load=true){
 ...
 goalsGeneration++;goalsAbort?.abort();$('goal-dialog').close();goalEditing=null;
 page=next;$('consolidated-panel').hidden=page!=='consolidated';$('branch').parentElement.hidden=page==='consolidated';salesPage=1;rankPage=1;customerPage=1;productPage=1;generation++;abort?.abort();
...
async function loadData(){
 if(!token)return;
 if(page==='consolidated'){await loadConsolidated();return;}
...
function renderSales(d){
 const body=$('sales-body');body.replaceChildren();body.closest('table').classList.toggle('quality-table',page==='quality');
 ...
 for(const o of d.operacoes){
  ...
  const action=el('td'),button=el('button','Ver detalhes →','operation-link');button.setAttribute('aria-label',`Ver detalhes da operação ${o.cod_operacao}`);button.addEventListener('click',()=>openDetail(o));action.append(button);if(page==='quality'&&isAdmin){const control=reprocessControl(o);if(control)action.append(control);}
  row.append(op,el('td',o.vendedor_nome||'Sem identificação'),el('td',number(o.quantidade)),el('td',money(o.valor_final_centavos)),status,action);['Operação / Data','Vendedor','Peças','Valor','Conferência','Ações'].forEach((label,i)=>row.children[i].dataset.label=label);body.append(row);
 }
 clearTimeout(reprocessTimer);if(page==='quality'&&isAdmin&&d.operacoes.some(o=>['pendente','processando'].includes(o.reprocessamento?.estado)))reprocessTimer=setTimeout(()=>{if(page==='quality'&&token&&!$('detail-dialog').open)loadData();},10000);
...
async function openDetail(o){
 clearTimeout(reprocessTimer);clearDetailMedia();detailAbort=new AbortController();
 ...
 try{
  ...
  if(isAdmin){const control=reprocessControl(d.operacao,true);if(control)content.append(control);}
...
$('detail-close').addEventListener('click',()=>$('detail-dialog').close());$('detail-dialog').addEventListener('close',()=>{detailGeneration++;clearTimeout(reprocessTimer);clearDetailMedia();document.body.classList.remove('modal-open');if(token&&isAdmin&&page==='quality')loadData();});
...
async function loadPermissions(){
 ...
 try{
  ...
  for(const perfil of data.perfis){
   ...
   for(const recurso of data.recursos){const label=el('label',null,'permission-check');const input=el('input');input.type='checkbox';input.value=recurso.codigo;input.checked=perfil.permissoes.includes(recurso.codigo);input.disabled=protectedRole||(['consolidado:ler','metas:gerenciar'].includes(recurso.codigo)&&!['Diretoria','Supervisao'].includes(perfil.role));label.append(input,el('span',recurso.nome));group.append(label);checks.set(recurso.codigo,input);}
   checks.get('metas:gerenciar').addEventListener('change',()=>{if(checks.get('metas:gerenciar').checked)checks.get('consolidado:ler').checked=true;});
   checks.get('consolidado:ler').addEventListener('change',()=>{if(!checks.get('consolidado:ler').checked)checks.get('metas:gerenciar').checked=false;});
   form.append(el('p','Consolidado e metas: Diretoria e Supervisão acessam somente filiais atribuídas ao usuário, mesmo se outros recursos estiverem liberados para todas as filiais.','muted permission-help'));
...
async function loadUsers(){
 ...
 try{
  ...
  function edit(user={email:'',role:'Vendas',active:true,vinculos:[]}){
   ...
   function update(){const r=roles.perfis.find(r=>r.role===role.value);help.textContent=self?'Seu próprio acesso é protegido. Peça a outro Admin para alterá-lo.':r.todas_filiais?'Este cargo acessa todas as filiais autorizadas da empresa.':r.somente_proprias_vendas?'Selecione as filiais e o vendedor de cada loja. A pessoa verá somente as próprias vendas.':'Selecione as filiais que esta pessoa pode consultar.';group.hidden=r.todas_filiais&&!['Diretoria','Supervisao'].includes(r.role);if(['Diretoria','Supervisao'].includes(r.role))help.textContent='Selecione as filiais que esta pessoa pode consultar no consolidado e nas metas.';for(const l of links){l.vendor.disabled=self||!l.check.checked;l.vendor.required=r.somente_proprias_vendas&&l.check.checked;}}
...
function reprocessControl(o,detail=false){
 if(!isAdmin||o.tipo_operacao!=='S')return null;
 const eligible=o.conciliacao!=='conciliada'&&!o.cancelada,r=o.reprocessamento,active=['pendente','processando'].includes(r?.estado);
 if(!eligible&&!r)return null;
 const box=el('div',null,'reprocess-control'),status=el('p',null,'reprocess-status');status.setAttribute('role','status');
 if(r){status.textContent=r.estado==='pendente'?'Reprocessamento aguardando na fila.':r.estado==='processando'?'Consultando novamente o ERP…':r.estado==='falhou'?'Falha ao reprocessar. Dados anteriores preservados.':r.resultado?.cancelada?'Reprocessada: venda cancelada.':r.resultado?.conciliacao==='conciliada'?'Reprocessada e conciliada.':'Reprocessada; confira a situação atual da venda.';if(r.finalizado_em)status.append(' '+when(r.finalizado_em));box.append(status);}
 if(eligible){const button=el('button',active?'Reprocessamento solicitado':'Reprocessar venda','button reprocess-sale');button.type='button';button.disabled=active;button.setAttribute('aria-label','Reprocessar venda '+o.cod_operacao);box.prepend(button);
 button.addEventListener('click',async()=>{if(!confirm('Buscar novamente a venda '+o.cod_operacao+' no ERP e atualizar os dados importados? O histórico anterior será guardado e a conferência será recalculada.'))return;button.disabled=true;status.textContent='Solicitando reprocessamento…';if(!status.isConnected)box.append(status);const version=generation,detailVersion=detailGeneration;try{const d=await api(operationPath(o)+'/reprocessar',{method:'POST',body:JSON.stringify({requisicao:crypto.randomUUID()})});if(!token||generation!==version||detail&&detailVersion!==detailGeneration)return;showToast('Reprocessamento solicitado. O resultado será atualizado nesta tela.');if(detail&&$('detail-dialog').open)await openDetail(o);else if(page==='quality')await loadData();}catch(e){status.textContent=e.message;button.disabled=false;}});}
 if(detail&&active){const version=detailGeneration;reprocessTimer=setTimeout(async()=>{if(!token||!isAdmin||!$('detail-dialog').open||version!==detailGeneration)return;try{const d=await api(operationPath(o)+'/reprocessar');if(version!==detailGeneration)return;if(d.reprocessamento?.estado===r.estado){const replacement=reprocessControl({...o,reprocessamento:d.reprocessamento},true);if(box.isConnected&&replacement)box.replaceWith(replacement);}else await openDetail(o);}catch(e){if(box.isConnected)status.textContent=e.message+' Feche e abra os detalhes para consultar novamente.';}},10000);}
 return box;
}

let goalsGeneration=0,goalsAbort=null,goalEditing=null;
function consolidatedMetrics(root,items){root.replaceChildren();for(const [label,value,note] of items){const card=el('article',null,'consolidated-metric');card.append(el('span',label),el('strong',value));if(note)card.append(el('small',note));root.append(card);}}
function labeledRow(cells,labels){const row=el('tr');cells.forEach((cell,i)=>{const td=cell instanceof HTMLElement?cell:el('td',cell);td.dataset.label=labels[i];row.append(td);});return row;}
async function loadConsolidated(){
 if(!filters)return;const version=++generation;abort?.abort();abort=new AbortController();resetError();busy(true);$('data-notice').hidden=true;$('consolidated-panel').hidden=true;
 try{
  const d=await api('/consolidado?'+new URLSearchParams({inicio:filters.inicio,fim:filters.fim}),{signal:abort.signal});if(version!==generation||page!=='consolidated')return;
  $('consolidated-rule').textContent=d.regra;$('scope-label').textContent=`${d.filiais.length} filiais autorizadas · ${dateBR(d.inicio)} a ${dateBR(d.fim)}`;$('last-sync').textContent='Atualização e cobertura indicadas por filial';
  consolidatedMetrics($('consolidated-metrics'),[['Valor de vendas',money(d.total.valor_centavos),'Período selecionado'],['Peças vendidas',number(d.total.pecas),'Quantidade do cabeçalho'],['Vendas',number(d.total.vendas),'Operações não canceladas'],['Filiais',number(d.filiais.length),'Somente seu escopo de acesso']]);
  $('consolidated-coverage').textContent=`${number(d.total.pendencias)} vendas com pendência de conferência. ${d.filiais_sem_cobertura?d.filiais_sem_cobertura+' filiais com cobertura parcial ou ainda não comprovada.':'Checkpoints cobrem o período; homologação com o ERP permanece necessária.'}`;
  const root=$('consolidated-body');root.replaceChildren();const labels=['Posição / Filial','Vendas','Peças','Valor de vendas','Participação','Conferência / Atualização'];
  for(const r of d.filiais){const name=el('td'),status=el('td');name.append(el('strong',r.posicao+'º · '+branchLabel(r)));status.append(el('span',r.cobertura_completa?'Cobertura do período':'Cobertura parcial','status-badge '+(r.cobertura_completa?'':'pending')),el('small',number(r.pendencias)+' pendências'),el('small',when(r.atualizado_em)));root.append(labeledRow([name,number(r.vendas),number(r.pecas),money(r.valor_centavos),r.participacao_percentual===null?'—':decimal(r.participacao_percentual)+'%',status],labels));}
  if(!d.filiais.length)emptyTable(root,6,'Nenhuma filial atribuída','Solicite a atribuição de filiais ao administrador.');
  $('consolidated-total').replaceChildren(labeledRow(['Total das filiais',number(d.total.vendas),number(d.total.pecas),money(d.total.valor_centavos),BigInt(d.total.valor_centavos)>0n?'100,00%':'—',''],labels));$('consolidated-panel').hidden=false;busy(false);await loadGoals();
 }catch(e){if(e.name==='AbortError'||version!==generation)return;busy(false);displayError(e);}
}
async function loadGoals(){
 const version=++goalsGeneration;goalsAbort?.abort();goalsAbort=new AbortController();$('goals-status').textContent='Consultando metas…';$('goals-body').replaceChildren();$('goals-metrics').replaceChildren();
 try{
  const d=await api('/metas?'+new URLSearchParams({competencia:$('goals-month').value}),{signal:goalsAbort.signal});if(version!==goalsGeneration||page!=='consolidated')return;
  $('goals-status').textContent=`Mês completo: ${dateBR(d.inicio)} a ${dateBR(d.fim)}. ${d.filiais_com_meta} de ${d.filiais.length} filiais com meta. Valores ainda não importados não entram no realizado. Devoluções aguardam homologação.`;
  consolidatedMetrics($('goals-metrics'),[['Metas cadastradas',money(d.meta_total_centavos),d.filiais_com_meta+' filiais'],['Vendas nas filiais com meta',money(d.realizado_com_meta_centavos),'Mês completo selecionado'],['Atingimento conjunto',d.atingimento_percentual===null?'—':decimal(d.atingimento_percentual)+'%','Apenas filiais com meta cadastrada']]);
  const labels=['Filial','Vendas do mês','Meta mensal','Atingimento','Falta para a meta','Ações'];
  for(const r of d.filiais){const name=el('td'),action=el('td'),progress=el('td');name.append(el('strong',branchLabel(r)));if(!r.cobertura_completa)name.append(el('small','Realizado com cobertura parcial'));progress.append(el('strong',r.atingimento_percentual===null?'—':decimal(r.atingimento_percentual)+'%'));if(r.atingimento_percentual!==null){const bar=el('progress');bar.max=100;bar.value=Math.max(0,Math.min(100,Number(r.atingimento_percentual)));bar.setAttribute('aria-label','Atingimento da meta de '+branchLabel(r));progress.append(bar);}
   if(d.pode_editar){const b=el('button',r.meta.valor_centavos===null?'Cadastrar meta':'Editar meta','button goal-edit');b.type='button';b.setAttribute('aria-label',(r.meta.valor_centavos===null?'Cadastrar':'Editar')+' meta de '+branchLabel(r));b.addEventListener('click',()=>openGoal(r,d.competencia));action.append(b);}else action.textContent='Consulta';
   $('goals-body').append(labeledRow([name,money(r.valor_centavos),r.meta.valor_centavos===null?'Não cadastrada':money(r.meta.valor_centavos),progress,money(r.falta_centavos),action],labels));
  }
  if(!d.filiais.length)emptyTable($('goals-body'),6,'Nenhuma filial atribuída','O administrador pode ajustar seu acesso.');
 }catch(e){if(e.name!=='AbortError'&&version===goalsGeneration)$('goals-status').textContent=e.message;}
}
$('goals-month').addEventListener('change',()=>{if(token&&page==='consolidated')loadGoals();});
function openGoal(row,mes){goalEditing={row,mes,requisicao:crypto.randomUUID()};$('goal-title').textContent=row.meta.valor_centavos===null?'Cadastrar meta':'Editar meta';$('goal-branch').value=branchLabel(row);$('goal-competence').value=mes;$('goal-value').value=row.meta.valor_centavos===null?'':(BigInt(row.meta.valor_centavos)/100n).toString()+','+String(BigInt(row.meta.valor_centavos)%100n).padStart(2,'0');$('goal-remove').hidden=row.meta.valor_centavos===null;$('goal-error').textContent='';$('goal-save').disabled=false;$('goal-remove').disabled=false;$('goal-dialog').showModal();$('goal-value').focus();}
function parseGoalValue(value){const v=value.trim();if(!/^(?:\d{1,12}|\d{1,3}(?:\.\d{3}){1,3})(?:,\d{1,2})?$/.test(v))throw new Error(errorText.META_INVALIDA);const [n,d='']=v.replaceAll('.','').split(',');return n+'.'+d.padEnd(2,'0');}
async function saveGoal(remover=false){
 if(!goalEditing)return;const edit=goalEditing;let valor;try{valor=remover?null:parseGoalValue($('goal-value').value);}catch(e){$('goal-error').textContent=e.message;return;}
 $('goal-save').disabled=true;$('goal-remove').disabled=true;$('goal-error').textContent='Salvando…';
 try{await api('/metas/'+edit.row.filial+'/'+edit.mes,{method:'PUT',body:JSON.stringify({valor,versao:edit.row.meta.versao,requisicao:edit.requisicao})});if(goalEditing!==edit||!token)return;$('goal-dialog').close();goalEditing=null;showToast(remover?'Meta removida; histórico preservado.':'Meta salva.');if(page==='consolidated')await loadGoals();}
 catch(e){if(goalEditing===edit){$('goal-error').textContent=e.message;if(e.message===errorText.META_DESATUALIZADA)await loadGoals();$('goal-save').disabled=false;$('goal-remove').disabled=false;}}
}
$('goal-form').addEventListener('submit',e=>{e.preventDefault();saveGoal();});$('goal-cancel').addEventListener('click',()=>$('goal-dialog').close());$('goal-dialog').addEventListener('close',()=>{goalEditing=null;});$('goal-remove').addEventListener('click',()=>{if(confirm('Remover a meta deste mês? O histórico das alterações será preservado.'))saveGoal(true);});
