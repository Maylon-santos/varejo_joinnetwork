...
export function normalizarVenda(v,filial,inicio,fim){
 ...
 const produtos=v.produtos.map((p,i)=>{
  ...
  return {imagem_url:imagemProduto(p),ordem:i,sku:p.sku??null,cod_produto:p.cod_produto==null?null:String(p.cod_produto),descricao:p.descricao??null,quantidade:p.quantidade,preco_centavos:centavos(p.preco).toString()};
 });
