...
export async function ambienteFila(){
 ...
 const auth=await criarAuth(control,'teste'),fila=criarFila(pool,'teste',filiais);server=criarServidor({auth,fila,painel:criarPainel(pool,'teste',filiais),filiais,gestaoPermissoes:criarGestaoPermissoes(control,'teste'),frontend:await criarFrontend(),limitar:()=>true,limitarLogin:()=>true});await new Promise(r=>server.listen(0,'127.0.0.1',r));
