...
try{
 const tenant=process.env.TENANT_KEY,filial=process.env.CLIENTES_FILIAL||'30098297',inicio=process.env.CLIENTES_INICIO||'2026-09-01',fim=process.env.CLIENTES_FIM||'2026-09-07';
 ...
 if(!/^\d{4}-\d{2}-\d{2}$/.test(inicio)||!/^\d{4}-\d{2}-\d{2}$/.test(fim)||inicio>fim||!(Date.parse(fim)-Date.parse(inicio)<=6*86400000))throw Error('PERIODO_INVALIDO');
 ...
 (SELECT md5(string_agg(md5((to_jsonb(o)-'clientes'-'clientes_importados_em'-'clientes_identidade_importada')::text),'' ORDER BY filial,tipo_operacao,cod_operacao)) FROM operacoes o) AS operacoes,
 ...
 const antes=await assinatura();const ops=await consultarVendas({baseUrl:process.env.MILLENNIUM_BASE_URL,token:process.env.MILLENNIUM_BASIC_TOKEN,filial,inicio,fim});
 const atualizadas=await repo.transacao({tenant,filial},tx=>tx.preencherIdentidadeClientes(ops));const preservado=antes===await assinatura();if(!preservado)throw new Error('BASE_COMERCIAL_ALTERADA');
 console.log(JSON.stringify({evento:'amostra_identidade_clientes',filial,inicio,fim,atualizadas,baseComercialECheckpointsPreservados:preservado}));
